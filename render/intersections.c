...
/*   Updated: 2023/07/11 09:17:29 by mfouadi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double    intersp(t_utils *utils, t_sphere *sp)
{
	...
	if(sol.delta > 0)
	{
		...
		((t < utils->T.t || utils->T.t == -1) && t > EPS) && (utils->T.t = t, utils->T.color = sp->color, 0);
	}
	else if(sol.delta == 0)
	{
		t = -sol.b / 2 * sol.a;
	...
	((t < utils->T.t || utils->T.t == -1) && t > EPS) && (utils->T.t = t, 0);
    return (t);
}